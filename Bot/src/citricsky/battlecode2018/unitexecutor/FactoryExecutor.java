...
public class FactoryExecutor implements UnitExecutor {
	...
	public UnitType getProduceType(MapLocation location) {
		...
		if (moveManager.nearEnemy(location.getPosition(), 6, true)) {
		...
		if (((double)RoundInfo.getUnitCount(UnitType.HEALER)) <
				((double)(RoundInfo.getCombatUnitsCount() - RoundInfo.getUnitCount(UnitType.HEALER))) / 2.0) {
			return UnitType.HEALER;
		}
